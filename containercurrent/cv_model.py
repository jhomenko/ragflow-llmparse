...
class AnthropicCV(Base):
    ...
    def describe_with_prompt(self, image, prompt=None):
        b64 = self.image2base64(image)
        vision_prompt = self.vision_llm_prompt(b64, prompt if prompt else vision_llm_describe_prompt())
 
        response = self.client.messages.create(model=self.model_name, max_tokens=self.max_tokens, messages=vision_prompt)
        return response["content"][0]["text"].strip(), response["usage"]["input_tokens"] + response["usage"]["output_tokens"]
...
class GoogleCV(AnthropicCV, GeminiCV):
    ...
    def chat(self, system, history, gen_conf, images=[], **kwargs):
        if "claude" in self.model_name:
            return AnthropicCV.chat(self, system, history, gen_conf, images, **kwargs)
        else:
            return GeminiCV.chat(self, system, history, gen_conf, images, **kwargs)

    def chat_streamly(self, system, history, gen_conf, images=[], **kwargs):
        if "claude" in self.model_name:
            for ans in AnthropicCV.chat_streamly(self, system, history, gen_conf, images, **kwargs):
                yield ans
        else:
            for ans in GeminiCV.chat_streamly(self, system, history, gen_conf, images, **kwargs):
                yield ans

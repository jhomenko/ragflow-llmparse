...
from deepdoc.parser.direct_vision_parser import DirectVisionParser
import trio
import numpy as np
...
class Parser(ProcessBase):
    ...
    def _pdf(self, name, blob):
        ...
        parse_method = (conf.get("parse_method") or "").lower()
        ...
        if parse_method == "deepdoc":
            ...
            # 1) Build the vision model
            vision_model = LLMBundle(
                self._canvas._tenant_id,
                LLMType.IMAGE2TEXT,
                llm_name=conf.get("parse_method"),
                lang=self._param.setups["pdf"].get("lang"),
            )

            # 2) Load the Markdown prompt from disk once
            #    - default path mirrors your project layout
            #    - allow override via setups.pdf.vision_prompt_path
            prompt_path_cfg = conf.get("vision_prompt_path")
            if prompt_path_cfg:
                prompt_path = Path(prompt_path_cfg)
            else:
                # rag/flow/parser/parser.py -> ../../prompts/vision_llm_describe_prompt.md
                prompt_path = (Path(__file__).resolve().parent.parent / "prompts" / "vision_llm_describe_prompt.md")

            try:
                prompt_text = prompt_path.read_text(encoding="utf-8")
            except Exception as e:
                logging.warning("Failed to read VLM prompt at %s: %s; proceeding with empty prompt",
                                prompt_path, e)
                prompt_text = ""

            # 3) Call VisionParser with prompt_text explicitly
            lines, _ = VisionParser(vision_model=vision_model)(
                blob,
                callback=self.callback,
                zoomin=conf.get("zoomin", 4),
                prompt_text=prompt_text,
            )

            # 4) Convert (text, meta) to bboxes
            bboxes = []
            for t, poss in lines:
                meta = poss.strip().lstrip("@").rstrip("#")
                fields = re.split(r"[\t ]+", meta)
                if len(fields) < 5:
                    logging.warning("Bad page geometry: %r", poss)
                    continue
                pn, x0, x1, top, bott = fields[:5]
                bboxes.append({
                    "page_number": int(float(pn)),
                    "x0": float(x0),
                    "x1": float(x1),
                    "top": float(top),
                    "bottom": float(bott),
        ...
        if conf.get("output_format") == "markdown":
            ...
            for b in bboxes:
                if b.get("layout_type", "") == "title":
                    mkdn += "\n## "
                if b.get("layout_type", "") == "figure" and b.get("image"):

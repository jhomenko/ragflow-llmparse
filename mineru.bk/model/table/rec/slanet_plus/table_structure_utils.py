...
class OrtInferSession:
    ...
    def _get_ep_list(self) -> List[Tuple[str, Dict[str, Any]]]:
        ...
        ov_device = os.getenv("MINERU_OPENVINO_DEVICE", "GPU")
        if EP.OPENVINO_EP.value in self.had_providers:
            self.use_cuda = False
            self.use_directml = False
            return [
                (EP.OPENVINO_EP.value, {"device_type": ov_device}),
                (EP.CPU_EP.value, cpu_provider_opts),
            ]
    ...
    def _check_cuda(self) -> bool:
        ...
        if isinstance(cur_device, str) and cur_device.startswith("cuda") and EP.CUDA_EP.value in self.had_providers:
            return True
